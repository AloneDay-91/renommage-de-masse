#  Renommage en masse

<p align="center">
  <a href="https://www.python.org/"><img src="https://img.shields.io/badge/Project-Info-blue?style=flat-square&logo=data:image/svg%2bxml;base64,"/></a>
  <a href="https://www.python.org/"><img src="https://img.shields.io/badge/Made%20with-Python-1f425f.svg"/></a>
  <a href="https://github.com/AloneDay-91/renommage-de-masse/releases"><img src="https://img.shields.io/github/downloads/AloneDay-91/renommage-de-masse/total.svg"/></a>
  <a href="https://github.com/AloneDay-91/renommage-de-masse/releases/tag/v1.0.0"><img src="https://badge.fury.io/gh/AloneDay-91%2Frenommage-de-masse.svg"/></a>
  <a href="https://github.com/ellerbrock/open-source-badges/"><img src="https://badges.frapsoft.com/os/v1/open-source.svg?v=103"/></a>
</p>

Le projet consiste a permettre à l'utilisateur de renommer ses fichiers rapidement.

## Installation
```bash
git clone https://github.com/AloneDay-91/renommage-de-masse
```
